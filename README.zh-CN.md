...
提取要求后，本地模型先审查地点限制，再启动检索。明确要求在西澳之外线下或强制 hybrid 办公，且没有西澳办公或从西澳远程选项时，流程立即中止（CLI 退出码 3），页面弹窗展示 JD 依据。地点不明确或仅为偏好时不自动拦截。审查依赖模型理解及提取出的地点要求；引用核验只能确认原文存在，不能保证语义判断准确。
